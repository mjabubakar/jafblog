...
export const editPost: any = async (req: any, res: any, next: any) => {
  ...
  const postUrl: string = req.params.url;
...
export const createPost: any = async (req: any, res: any, next: any) => {
  ...
  const postUrl: string = url(title) + "-" + generateRand(12);
...
export const myPosts = async (req: any, res: any, next: any) => {
  ...
  const total = await Post.countDocuments({ userId: user.id });
  const count: number = Math.ceil(total / ITEMS_PER_PAGE);
  const allPosts = await Post.find({ userId: user.id }, postData)
    .skip((page - 1) * ITEMS_PER_PAGE)
    .limit(ITEMS_PER_PAGE);

  res.status(200).send({ posts: allPosts, total: count });
};
...
export const posts = async (req: any, res: any, next: any) => {
  try {
    ...
    const len: number = Math.ceil(count / ITEMS_PER_PAGE);
...
export const post = async (req: any, res: any, next: any) => {
  ...
  const { fullname, profilepic, bio }: IUser = user;
  ...
  if (nextPost.length !== 0)
    data.nextPost = { title: nextPost[0].title, url: nextPost[0].url };
